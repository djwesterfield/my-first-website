...
var MOOdalBox = {
	...
	init: function (options) {
		...
		// the center element
		this.center = new Element('div').setProperty('id', 'mb_center').setStyles({width: this.options.initialWidth+'px', height: this.options.initialHeight+'px', 'z-index': '999', marginLeft: '-'+(this.options.initialWidth/2)+'px', display: 'none'}).injectInside(document.body);
	...
	nextEffect: function() {
		...
		if(this.step == 0)
		{
			if($('date_starts')) { var myCal = new Calendar({ date_starts: 'm-d-Y'}, { direction: 1 }); } 
		}
